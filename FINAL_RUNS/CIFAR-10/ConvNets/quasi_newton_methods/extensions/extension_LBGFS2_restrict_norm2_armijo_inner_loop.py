...
#        self.method = self._parameter_updaters[0].method
    ...
    def get_gradient2(self):
        ...
        for _ in range(self.calculating_gradient_steps):
            cost_args = self.training_iterator.next()
        if self.armijo:
            cost_batch = self.function_value(*cost_args)
            gradient += (self.gradient_function(*cost_args)/self.calculating_gradient_steps)

        if self.armijo:
            return gradient, cost_args, cost_batch
        else:
            return gradient
    ...
    def LBFGS_step(self):
        ...
        if self.armijo:
                grad, cost_args, cost_batch = self.get_gradient2()
        else:
        ...
        if self.armijo == True: # Perform line search
            ...
            while not alpha_found:
                ...
                #validation_function_value = self.get_validation_function_value()
                new_cost_batch = self.function_value(*cost_args)

                val2 = cost_batch - (beta_ * learning_rate * numpy.dot(grad,direction))
                # Do the actual backtracking:
                if new_cost_batch < val2+tolerance or inner_loop_counter > 20:
                    alpha_found = True
                else:
