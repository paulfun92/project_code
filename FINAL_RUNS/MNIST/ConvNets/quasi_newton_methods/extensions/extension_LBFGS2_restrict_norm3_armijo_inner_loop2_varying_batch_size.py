...
class Bgfs(object):
    ...
    def __init__(self,
        ...
        self.total_size_dataset = self.training_iterator.dataset.tensors[0].shape[0]
        self.batches_in_epoch = self.total_size_dataset / batch_size

        self.batch_size = batch_size
        self.batch_size_for_calculation =self.training_iterator.batch_size
        assert_less_equal(self.batch_size_for_calculation, batch_size)
        ...
        # for validation line search:
        total_size_validation_dataset = self.validation_iterator.dataset.tensors[0].shape[0]
        self.batch_size_validation = self.validation_iterator.batch_size
        self.batches_in_epoch_validation = total_size_validation_dataset / self.batch_size_validation
    ...
    def get_gradient2(self):
        ...
        if self.current_batch_counter > 19 or self.current_batch_counter == 0:
            self.y = []
            self.s = []
            self.rho = []
            self.grad_log = []
            ...
            for _ in range(self.calculating_gradient_steps):
                self.current_batch.append(self.training_iterator.next())

            self.current_batch_counter = 0

        for batch in self.current_batch:
            gradient += (self.gradient_function(*batch)/len(self.current_batch))

        if self.batch_size != 50000:
            self.current_batch_counter += 1
        elif self.current_batch_counter == 0:
            self.current_batch_counter = 1
        ...
        if self.armijo:
            ...
            for batch in self.current_batch:
                cost_batch += (self.function_value(*batch) / len(self.current_batch))

            return gradient, cost_batch
        else:
        '''
        return gradient
    ...
    def LBFGS_step(self):
        ...
        '''
        if self.armijo:
            grad, cost_batch = self.get_gradient2()
        else:
        '''
        ...
        if self.armijo == True: # Perform line search
            ...
            while not alpha_found:
                ...
                validation_func_value_old = self.get_validation_function_value()
                ...
                for batch in self.current_batch:
                    new_cost_batch += (self.function_value(*batch) / len(self.current_batch))
                '''
                validation_func_value_new = self.get_validation_function_value()

                '''
                val2 = cost_batch - (beta_ * learning_rate * numpy.dot(grad,direction))
                # Do the actual backtracking:
                if new_cost_batch < val2+tolerance or inner_loop_counter > 10:
                    alpha_found = True
                    print("Learning rate used is ", learning_rate)
                else:
                    learning_rate = tau*learning_rate
                    inner_loop_counter += 1
                '''

                val2 = validation_func_value_old - (beta_ * learning_rate * numpy.dot(grad,direction))
                # Do the actual backtracking:

                if validation_func_value_new  < val2+tolerance or inner_loop_counter > 15:
                    alpha_found = True
                    print("Learning rate used is ", learning_rate)
                else:
                    learning_rate = tau*learning_rate
