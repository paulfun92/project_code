...
#        self.method = self._parameter_updaters[0].method
    ...
    def get_gradient2(self):
        ...
        if self.current_batch_counter > 2 or self.current_batch_counter == 0:

            self.k = 0 #counter
            self.current_batch = self.training_iterator.next()
            self.current_batch_counter = 0
            self.previous_gradient = 0
            self.previous_direction = 0

        for _ in range(self.calculating_gradient_steps):
            gradient += (self.gradient_function(*self.current_batch)/self.calculating_gradient_steps)

        self.current_batch_counter += 1

        if self.armijo:
            cost_batch = self.function_value(*self.current_batch)
    ...
    def CG_step(self):
        ...
        if self.k == 0:
            ...
            elif self.method == 'PR_plus':
                beta = max( numpy.dot(grad, (grad-self.previous_gradient)) / numpy.dot(self.previous_gradient, self.previous_gradient), 0 )
            elif self.method == 'HS':
        ...
        if self.armijo == True: # Perform line search
            ...
            while not alpha_found:
                ...
                #validation_function_value = self.get_validation_function_value()
                new_cost_batch = self.function_value(*self.current_batch)
