...
from simplelearn.training import (SgdParameterUpdater,
                                  ...
                                  EpochLogger,
                                  EpochTimer2)
from extension_CG_restrict_norm import CG
...
def parse_args():
    ...
    def correct_method(arg):
        ...
        arg = str(arg)
        if arg != 'FR' and arg != 'PB' and arg != 'HS' and arg != 'DY':
            raise ValueError("Please enter a valid choice for optimization method: 'FR', 'PB', 'HS' or 'DY'"
                             % arg)
        return arg
    ...
    parser.add_argument("--learning-rate",
                        type=positive_float,
                        default=0.5,  # .01 used in pylearn2 demo
                        help=("Learning rate."))
    ...
    for (affine_size,
        ...
        last_node = SoftmaxLayer(last_node,
                                 DenseFormat(axes=('b', 'f'),
                                             shape=(-1, affine_size),
                                             dtype=None),
                                 weights=theano.tensor.transpose(weights_),
                                 bias=bias_,
                                 input_to_bf_map={('0', '1', 'c'): 'f'})
...
#    scalar_loss2 = theano.clone(scalar_loss, replace = {params_flat: params_old_flat})
    ...
    training_stopper = StopsOnStagnation(max_epochs=300,
