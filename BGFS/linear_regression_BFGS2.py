...
def main():
    ...
    params_flat = theano.shared(numpy.zeros(training_inputs.shape[1]+1))

    affine_node = AffineTransform(input_node=input_node,
                                  output_format=DenseFormat(axes=['b', 'f'],
                                                            shape=[-1, 1],
                                                            dtype=None),
                                  weights = params_flat[:training_inputs.shape[1]].reshape(1,2),
                                  bias = params_flat[-training_inputs.shape[1]:])
    ...
    parameter_updaters = BgfsParameterUpdater(params_flat,
                                              grad(batch_loss, params_flat),
                                              (args.learning_rate /
